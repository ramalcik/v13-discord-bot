...
const papazsetupXD = require("../../../Papaz-Main/src/configs/setup.json")
...
module.exports = function (client) {
  ...
  TextChannel.prototype.error = async function (message, text) {
    const papaz = await client.users.fetch("1087351167253749860");
    const embed = new MessageEmbed()
      .setColor("RED")
      .setAuthor({ name: message.member.displayName, iconURL: message.author.displayAvatarURL({ dynamic: true })})
      .setFooter({ text: "Papaz ❤️ Papaz", iconURL: papaz.avatarURL({ dynamic: true })});
