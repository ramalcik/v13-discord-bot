...
module.exports = async (message) => {
  ...
  let prefix = settings.Main.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  if (message.author.bot || !message.guild || !prefix || conf.jailRole.some(x => message.member.roles.cache.has(x))) return;
  let args = message.content.substring(prefix.length).trim().split(" ");
  let commandName = args[0].toLowerCase();
  
  const embed = new MessageEmbed().setFooter({ text: settings.AltBaşlık}).setColor(message.member.displayHexColor) ;
  ...
  if (cmd) {
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [papaz]})
  }
};
