...
if(button.customId === "grs") {
  ...
  .setDescription(`
\`\`\`\nSUNUCU-GİRİŞ\n\`\`\`
Toplam ${(message.guild.memberCount)} kullanıcı sunucumuzda bulunmaktadır.
Sunucumuza **1 saat** içerisinde ${(message.guild.members.cache.filter(papaz => (new Date().getTime() - papaz.joinedTimestamp) < 1000 * 60 * 60).size)} kullanıcı giriş yapmış.
Sunucumuza **1 gün** içerisinde ${(message.guild.members.cache.filter(papaz => (new Date().getTime() - papaz.joinedTimestamp) < 1000 * 60 * 60 * 24).size)} kullanıcı giriş yapmış.
Sunucumuza **1 hafta** içerisinde ${(message.guild.members.cache.filter(papaz => (new Date().getTime() - papaz.joinedTimestamp) < 1000 * 60 * 60 * 24 * 7).size)} kullanıcı giriş yapmış.
Sunucumuza **1 ay** içerisinde ${(message.guild.members.cache.filter(papaz => (new Date().getTime() - papaz.joinedTimestamp) < 1000 * 60 * 60 * 24 * 30).size)} kullanıcı giriş yapmış.
`)
.setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
