    ...
    let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2])
        ...
        if (args[0] == "al") {
          if (rMember.roles.cache.has(role.id)) {
            ...
            message.reply({ embeds: [embed.setDescription(`Başarıyla ${rMember} Kişisinden ${role} rolünü aldım.`)]})
          } else {
            message.reply({ embeds: [embed.setDescription(`${rMember} Kişisinde ${role} rolü yok!`)]})
          }
      }
      if (args[0] == "ver") {
          if (!rMember.roles.cache.has(role.id)) {
