...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!papaz.chatMute.some(x => member.roles.cache.has(x))) {
        mute.setStyle('SECONDARY').setDisabled(true);
    } else {
        mute.setStyle('SUCCESS');
    }

    if (!papaz.voiceMute.some(x => member.roles.cache.has(x))) {
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "mute") {
      ...
      if (conf.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
  ...
  .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
   await message.guild.channels.cache.get(conf.cmuteLogChannel).wsend({ embeds: [log1]});
    ...
    if (button.customId === "vmute") {
      ...
      member.roles.remove(papaz.voiceMute);
  ...
  .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
   await message.guild.channels.cache.get(papaz.vmuteLogChannel).wsend({ embeds: [log]});
