  ...
  run: async (client, message, args, perm, prefix) => {
        ...
        const data = await bannedTag.findOne({ guildID: settings.guildID });
                ...
                if (!data) {
                   ...
                   await bannedTag.findOneAndUpdate({ guildID: message.guild.id }, { $push: { tags: args[1] } }, { upsert: true });
                    ...
                    üyeler.map(x => {
                        ...
                        setTimeout(() => {
                            x.setNickname('Yasaklı Tag');
                            x.roles.cache.has(conf.boosterRolu) ? x.roles.set([conf.boosterRolu, papaz.jailRole[0]]) : x.roles.set(conf.jailRole)
                        }, 1000)
            ...
            if (args[0] == "liste" && !args[1]) {
                ...
                let num = 1
                let arrs = data.taglar.map(x => `\`${num++}.\` ${x} - (${client.users.cache.filter(s => data.taglar.some(conf => s.tag.includes(conf))).size} üye)`)
            ...
            if (args[0] == "kaldır") {
                ...
                üyeler.map(x => {
                    setTimeout(async () => {
                    x.setNickname(`${papaz.ikinciTag} İsim ' Yaş`);
                    x.roles.cache.has(papaz.boosterRolu) ? x.roles.set([conf.boosterRolu, papaz.unregRoles[0]]) : x.roles.set(conf.unregRoles)
                    }, 1000);
            ...
            if (args[0] == "kontrol") {
                ...
                data.taglar.forEach(x => {
                    ...
                    for (let i = 0; i < üye.length;i++) {
                        setTimeout(() => {
                            message.guild.members.cache.get(üye[i]).roles.cache.has(conf.boosterRolu) ? message.guild.members.cache.get(üye[i]).roles.set([papaz.boosterRolu, papaz.jailRole[0]]) : message.guild.members.cache.get(üye[i]).roles.set(papaz.jailRole)
                        }, (i + 1) * 1000)
                    }
                })
            }
