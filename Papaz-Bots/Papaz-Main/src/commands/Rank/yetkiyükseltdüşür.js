...
const inviterData = await inviterSchema.findOne({ guildID: message.guild.id, userID: member.user.id });
    const total = inviterData ? inviterData.total : 0;
      ...
      const coinStatus = message.member.hasRole(papaz.staffs, false) && client.ranks.length > 0 ?
      `${currentRank ?`
 ...
 if(!member.roles.cache.has(ranks[ranks.length-1])){
    ...
    button.reply({ content:`:x: Belirtilen Kullanıcı Zaten Max Role Sahip.`, ephemeral: true})}
      ...
      if(button.customId === "düşür") {
        ...
        ${member} adlı kullanıcının Yetkisi: ${papaz.yetkiRolleri.length > 1 ? papaz.yetkiRolleri.slice(0, -1).map(x => `<@&${x}>`).join(", ") + " ve " + papaz.yetkiRolleri.map(x => `<@&${x}>`).slice(-1) : papaz.yetkiRolleri.map(x => `<@&${x}>`).join("")}`)
      ...
      member.roles.remove(papaz.yetkiRolleri)
