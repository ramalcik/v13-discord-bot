...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
        if (!papaz.staffs.some(x => message.member.roles.cache.has(x))) return message.react(red)
...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      await coin.findOneAndUpdate({ guildID: member.guild.id, userID: message.author.id }, { $inc: { coin: settings.Main.dmMessagesyetkiCoin } }, { upsert: true });       
      ...
      member.roles.add(papaz.yetkiRolleri)
