...
Hesap İsmi: ${üye}
Hesap Kuruluş Tarihi: <t:${Math.floor(member.user.createdTimestamp / 1000)}:R>
`)
...
`);
  if (üye.permissions.has("ADMINISTRATOR") || Ayarlar.teyitciRolleri.some(x => üye.roles.cache.has(x))) 
    embed.addField(`❯ Kayıt Bilgileri`,
`Kayıt Sayısı: ${registerData ? registerData.top : 0} • Erkek kayıdı : ${registerData ? registerData.erkek : 0} • Kadın kayıdı : ${registerData ? registerData.kız : 0} •`)
  ...
  collector.on("collect", async (menu) => {
     if(menu.values[0] === "avatar") {
      ...
      async function bannerXd(user, client) {
        ...
        if(!response.data.banner) return `Etiketlediğiniz kişinin banneri yok!`
