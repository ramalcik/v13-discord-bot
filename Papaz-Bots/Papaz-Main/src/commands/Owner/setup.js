...
const papazsetupxd = new Database("/src/configs/setup.json");
const ayar = require("../../configs/setup.json");
...
Tag: (\` ${ayar.tag.length > 0 ? `${ayar.tag.map(x => `${x}`).join(",")}` : "\`YOK\`"} \`) / (\` ${ayar.ikinciTag ? ayar.ikinciTag : "YOK"} \`)
...
Welcome Channel: (${ayar.welcomeChannel.length ? `<#${ayar.welcomeChannel}>` : "\`YOK\`"})
...
/////
const tagsetup = [
  { name: ["tag"], conf: "tag", cmdName: "Tag(ları)" },
]
...
const setup4 = [
  ...
  { name: ["welcome","register","welcomechannel","welcomeChannel"], conf: "welcomeChannel", cmdName: "Hoşgeldin Kanal" },
...
tagsetup.forEach(async (x) => {
  ...
  let db = papazsetupxd.get(`${x.conf}`)
  if(tag) {
  if(db.some(papaz => papaz.includes(tag))) {
    papazsetupxd.pull(`${x.conf}`, `${tag.map(x => x)}`)
...
setup1.forEach(async (x) => {
  ...
  return }
  papazsetupxd.set(`${x.conf}`, `${select}`)
...
setup2.forEach(async (x) => {
  ...
  let db = papazsetupxd.get(`${x.conf}`)
  if(rol) {
  if(db.some(papaz => papaz.includes(rol.id))) {
    papazsetupxd.pull(`${x.conf}`, `${rol.map(x => x)}`)
...
setup3.forEach(async (x) => {
  if(x.name.some(x => x === choose)) {
  let rol = message.mentions.roles.first() || message.guild.roles.cache.get(args[2]) || message.guild.roles.cache.find(papaz => papaz.name === args.join(" "))
  if (rol) {
    papazsetupxd.set(`${x.conf}`, `${rol.id}`)
...
setup4.forEach(async (x) => {
  if(x.name.some(x => x === choose)) {
  let channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[2]) || message.guild.channels.cache.find(papaz => papaz.name === args.join(" "))
  if (channel) {
    papazsetupxd.set(`${x.conf}`, `${channel.id}`)
...
setup5.forEach(async (x) => {
  ...
  let db = papazsetupxd.get(`${x.conf}`)
  if(kanal) {
  if(db.some(papaz => papaz.includes(kanal.id))) {
    papazsetupxd.pull(`${x.conf}`, `${kanal.map(x => x)}`)
...
setup6.forEach(async (x) => {
  if(x.name.some(x => x === choose)) {
  let papaz = message.mentions.channels.first() || message.guild.channels.cache.get(args[1])
  if (!papaz) return message.reply({ content: `Sunucu **${x.cmdName}** belirtmelisin`, ephemeral: true })
  papazsetupxd.set(`${x.conf}`, papaz.id)
  message.reply({ content: `**${papaz}** ${x.cmdName} listesine başarıyla eklendi.`, ephemeral: true });
};
});
